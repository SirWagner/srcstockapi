...
namespace Stocks.API.Controllers
{
    ...
    public class ProductsController : ControllerBase
    {
        ...
        public ProductsController(BTContext db)
        {
            _db = db;
        }
        ...
        public async Task<IActionResult> Get()
        {
            try
            {
                var units = await _db.Artigo.ToListAsync();
                return Ok(units);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> GetById(int id)
        {
            try
            {
                var unit = await _db.Product.FindAsync(id);
                if (unit == null)
        ...
        public async Task<IActionResult> GetByWarehouse([FromQuery] string codBarras)
        {
            try
            {
                var units = await _db.Artigo.Where(p => p.CodBarras == codBarras).ToListAsync();
                return Ok(units);
            }
            catch (Exception ex)
            {
        ...
        public JsonResult Upsert(Artigo product)
        {
            ...
            {
                var productid = _db.Product.Find(product.IdEstado);
                if (productid == null)
